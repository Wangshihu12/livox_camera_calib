...
// instrins matrix 内参矩阵
Eigen::Matrix3d inner;
// Distortion coefficient 畸变系数
...
public:
  ...
  bool operator()(const T *_q, const T *_t, T *residuals) const {
    Eigen::Matrix<T, 3, 3> innerT = inner.cast<T>();    // 内参矩阵
    Eigen::Matrix<T, 4, 1> distorT = distor.cast<T>();  // 畸变系数
...
// 调整旋转参数来优化标定参数
void roughCalib(Calibration &calibra, 
                Vector6d &calib_params,     // 当前的标定参数，包括三个旋转角度和三个平移参数
                double search_resolution,   // 搜索的分辨率，即每次调整角度的步长
                int max_iter) {             // 每次角度调整的最大迭代次数
  float match_dis = 25;
  Eigen::Vector3d fix_adjust_euler(0, 0, 0);
  for (int n = 0; n < 2; n++) {
    for (int round = 0; round < 3; round++) { // 对每个旋转角度轴单独调整
      Eigen::Matrix3d rot;
